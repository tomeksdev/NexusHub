...
describe("parseProm", () => {
  it("skips HELP/TYPE comment lines", () => {
    const text = ["# HELP foo how many", "# TYPE foo counter", "foo 42"].join(
      "\n",
    );
  ...
  it("parses labels with commas and spaces", () => {
    const text =
      'nexushub_http_requests_total{method="GET",route="/peers",status="200"} 17';
...
describe("sum", () => {
  ...
  it("filters by label predicate", () => {
    expect(
      sum(samples, "nexushub_http_requests_total", (l) =>
...
describe("value", () => {
  const samples = parseProm(
    ["nexushub_db_pool_acquired_conns 3", "nexushub_db_pool_max_conns 25"].join(
      "\n",
    ),
  );
