...
export function setTokens(access: string, accessExpires: string, refresh?: string) {
  accessToken = access
...
async function refresh(): Promise<void> {
  ...
  if (!rt) throw new ApiError(401, { error: 'no refresh token', code: 'UNAUTHORIZED' })
  ...
  if (!resp.ok) {
    clearTokens()
    const body = await resp.json().catch(() => ({ error: 'refresh failed', code: 'UNAUTHORIZED' }))
    throw new ApiError(resp.status, body)
  }
  const data: { access_token: string; access_expires_at: string; refresh_token: string } =
    await resp.json()
