...
- [ ] Example programs: peer ingress filter, bandwidth meter, connection counter <!-- partial 2026-04-21: ebpf/src/rules.c is the XDP full-gate — LPM src → rule_meta → protocol + TCP/UDP port-range match → action switch, with IPv4 token-bucket rate-limiting via rate_state_v4 PERCPU_HASH. IPv6 rate-limiting and TC-on-wg0 programs pending. -->
