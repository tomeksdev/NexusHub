...
- [x] E2E tests (Playwright) covering: login, peer create, rule attach, audit view (2026-04-24) <!-- completed 2026-04-24: tests/e2e is a self-contained Playwright project (package.json + playwright.config.ts + .gitignore + README). Smoke suite covers the no-auth surface (/health, /openapi.yaml, unauthenticated root renders login). Auth suite covers bad-credentials + seeded-admin login landing on Peers with aria-current=page. .github/workflows/e2e-tests.yml brings the compose stack up, seeds a super_admin host-side via `go run ./cmd/seed` against the exposed postgres, installs chromium via npm, runs tests with traces on retry / screenshots on failure / videos retained on failure, and uploads the HTML report + compose logs as 7-day artefacts. Credentials are env-driven (NEXUSHUB_ADMIN_{EMAIL,USERNAME,PASSWORD}) and shared between workflow + tests so rotating is a one-place change. peer-create + rule-attach + audit-view suites ship as follow-up PRs — the skeleton + workflow + auth smoke is the scoped landing. -->
...
- [x] Multi-arch (`amd64`, `arm64`) image published to `ghcr.io/tomeksdev/nexushub` (2026-04-24) <!-- completed 2026-04-24: .github/workflows/docker-publish.yml drives the multi-arch build via docker/setup-qemu-action + docker/setup-buildx-action, tags via docker/metadata-action (semver patch/minor/major + latest), and pushes to ghcr.io/tomeksdev/nexushub on every v*.*.* tag. Dockerfile HEALTHCHECK fix: curl is now apt-installed — without it the -slim base exits 127 and containers read as unhealthy indefinitely. -->
- [x] Systemd unit files for bare-metal installs (2026-04-24) <!-- completed 2026-04-24: deploy/systemd/nexushub-api.service — runs as nexushub user, EnvironmentFile=/etc/nexushub/env, ExecStartPre modprobe wireguard (soft-fail), AmbientCapabilities=CAP_NET_ADMIN+CAP_BPF+CAP_NET_RAW with full bounding-set + NoNewPrivileges, ProtectSystem=strict + ProtectHome + PrivateTmp, crash-only Restart=on-failure with 3-per-60s burst cap, 20s graceful SIGTERM window matching the backend's own shutdown timeout. deploy/systemd/env.example is the chmod-600 template install.sh drops into /etc/nexushub/env on first run. -->
- [x] `install.sh` replacement for v1.0.0 (still supports one-liner bootstrap) (2026-04-24) <!-- completed 2026-04-24: scripts/install.sh — Debian+Ubuntu, amd64+arm64. One-liner: `curl -fsSL https://raw.githubusercontent.com/tomeksdev/NexusHub/main/scripts/install.sh | sudo NEXUSHUB_VERSION=v2.0.0 bash`. Idempotent: creates user + state dirs + postgres user/db only when missing, downloads the tar.gz for the requested tag (or latest via GitHub API), installs the binary + unit + env template, runs systemctl daemon-reload + enable, prints the next-step checklist. /etc/nexushub/env is preserved across re-runs — secrets never get overwritten by a reinstall. -->
- [x] Example `docker-compose.yml` for operators with TLS via Caddy (2026-04-24) <!-- completed 2026-04-24: docker/docker-compose.prod.yml with four services — postgres, api (NET_ADMIN+BPF only, frontend-dist volume mounted read-only), frontend-init (copies /app/frontend/dist into a named volume when the NEXUSHUB_TAG marker changes), caddy 2-alpine (80+443 exposed, Caddyfile + dist volume + ACME data volume mounted). docker/Caddyfile rewrites /api/* to api:8080, serves the SPA bundle from /srv/nexushub with try_files so deep links fall through to index.html, cache-control aggressive on /assets/* (content-hashed) and no-cache on index.html. docker/.env.example documents every required secret + hostname. -->
- [x] Kubernetes manifests + Helm chart (`deploy/helm/`) (2026-04-25) <!-- completed 2026-04-25: deploy/helm/nexushub — Chart.yaml v2 (kubeVersion >=1.26) + values.yaml documenting every field inline + _helpers.tpl + Deployment + Service + optional Ingress + Secret (chart-managed with helm.sh/resource-policy: keep so uninstall doesn't delete PEER_KEY_ENCRYPTION_KEY) + ConfigMap + ServiceAccount (no API access by default) + pre-install/pre-upgrade migrate Job hook + NOTES.txt + chart README. Pod runs runAsNonRoot 10001/10001, readOnlyRootFilesystem with /tmp emptyDir, capabilities drop ALL by default. dataPlane.enabled toggles hostNetwork + NET_ADMIN/BPF/NET_RAW back on for operators who want to run the kernel sync inside k8s (default off — recommended pattern is API-in-k8s + systemd unit on WireGuard hosts). secrets.existingSecret + postgres.existingSecret are the production paths; in-values are evaluation-only. checksum/config + checksum/secrets pod annotations force a rolling restart on config-only helm upgrades. Companion Dockerfile change ships /app/nexushub-migrate + /app/nexushub-seed alongside the API binary so the migrate Job + seed kubectl-run example actually work. helm not available in this env so chart wasn't lint/template-validated; first install attempt is the real verification. -->
- [x] Documented backup/restore procedure (2026-04-24) <!-- completed 2026-04-24: scripts/backup.sh + scripts/restore.sh auto-detect deployment shape (compose vs bare-metal systemd) and call pg_dump/psql against the right target. backup.sh keeps the most recent 14 dumps per directory and writes timestamped .sql.gz; restore.sh refuses to overwrite a non-empty DB without --force and drops public schema cleanly when forced. docs/deployment/backup-restore.md covers what to back up (DB + PEER_KEY_ENCRYPTION_KEY separately — DB-only backup is useless without the AEAD key), systemd timer + cron scheduling snippets, PITR caveats, and a quarterly DR-drill checklist. -->
